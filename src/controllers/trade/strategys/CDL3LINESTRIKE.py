...
from src.controllers.settings.strategys.set_strategys import CDL3LINESTRIKE as CDL3LINESTRIKE_strat
...
class CDL3LINESTRIKE():
    def __init__(self,df,regime='one_set',number_set = 0):
        ...
        self.data_mas = []
        if regime=='one_set':
            self.var = CDL3LINESTRIKE_strat()
        elif regime=='much_set':
            self.var = CDL3LINESTRIKE_strat(regime,number_set)
        
    def check_cignal(self):
        try:
            res = abstract.CDL3LINESTRIKE(self.df)
            # print(res[-1])
