...
from src.controllers.settings.strategys.set_strategys import CDLMATCHINGLOW as CDLMATCHINGLOW_strat
...
class CDLMATCHINGLOW():
    def __init__(self,df,regime='one_set',number_set = 0):
        ...
        self.data_mas = []
        if regime=='one_set':
            self.var = CDLMATCHINGLOW_strat()
        elif regime=='much_set':
            self.var = CDLMATCHINGLOW_strat(regime,number_set)
        
    def check_cignal(self):
        try:
            res = abstract.CDLMATCHINGLOW(self.df)
            # print(res[-1])
