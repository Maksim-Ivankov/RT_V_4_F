...
from src.controllers.settings.strategys.set_strategys import CDLHARAMI as CDLHARAMI_strat
...
class CDLHARAMI():
    def __init__(self,df,regime='one_set',number_set = 0):
        ...
        self.data_mas = []
        if regime=='one_set':
            self.var = CDLHARAMI_strat()
        elif regime=='much_set':
            self.var = CDLHARAMI_strat(regime,number_set)
        
    def check_cignal(self):
        try:
            res = abstract.CDLHARAMI(self.df)
            # print(res[-1])
