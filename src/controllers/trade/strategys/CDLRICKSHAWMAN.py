...
from src.controllers.settings.strategys.set_strategys import CDLRICKSHAWMAN as CDLRICKSHAWMAN_strat
...
class CDLRICKSHAWMAN():
    def __init__(self,df,regime='one_set',number_set = 0):
        ...
        self.data_mas = []
        if regime=='one_set':
            self.var = CDLRICKSHAWMAN_strat()
        elif regime=='much_set':
            self.var = CDLRICKSHAWMAN_strat(regime,number_set)
        
    def check_cignal(self):
        try:
            res = abstract.CDLRICKSHAWMAN(self.df)
            # print(res[-1])
