...
class Generate_TEMA_set(ft.UserControl):
    def __init__(self,btn_close):
        super().__init__()
        self.btn_close = btn_close
        
        
    def input_strat_TEMA_timeperiod(self,e):
        self.set_TEMA_timeperiod = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_TEMA_timeperiod':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_TEMA_set):
            os.remove(path_ini_TEMA_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'timeperiod':str(random.choice(self.set_TEMA_timeperiod.split(',')))},path_ini_TEMA_set)
    ...
    def print_page(self):
        ...
        self.set_TEMA_timeperiod = config.get('param_trade_historical_trade_svobodniy_freym', 'set_TEMA_timeperiod')
                                        ...
                                        ft.Container(
                                            ft.Container(
                                                    ...
                                                    ft.Column(
                                                        controls=[
                                                            ft.Container(
                                                                ft.Container(ft.Text('Тройная эксп сколь средняя',color=c_blue,),bgcolor=c_yelow,padding=5,margin=ft.margin.only(bottom=-10),border=ft.border.all(1,c_white))
                                                            ),
                                                            ft.Container(
                                                                ft.Container(
                                                                        ft.Column(controls=[
                                                                        ft.Container(ft.Text('Временной промежуток',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_strat_TEMA_timeperiod,self.set_TEMA_timeperiod,510),
                                                                ])),
                                                                width=550,
