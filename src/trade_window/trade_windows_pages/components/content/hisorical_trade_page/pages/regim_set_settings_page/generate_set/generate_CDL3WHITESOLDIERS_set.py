...
class Generate_CDL3WHITESOLDIERS_set(ft.UserControl):
    def __init__(self,btn_close):
        super().__init__()
        self.btn_close = btn_close
        
    def input_CDL3WHITESOLDIERS_long(self,e):
        self.set_CDL3WHITESOLDIERS_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDL3WHITESOLDIERS_long':str(e.control.value)})
    def input_CDL3WHITESOLDIERS_short(self,e):
        self.set_CDL3WHITESOLDIERS_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDL3WHITESOLDIERS_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDL3WHITESOLDIERS_set):
            os.remove(path_ini_CDL3WHITESOLDIERS_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDL3WHITESOLDIERS_long.split(',')))},path_ini_CDL3WHITESOLDIERS_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDL3WHITESOLDIERS_short.split(',')))},path_ini_CDL3WHITESOLDIERS_set)
    ...
    def print_page(self):
        ...
        self.set_CDL3WHITESOLDIERS_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDL3WHITESOLDIERS_long')
        self.set_CDL3WHITESOLDIERS_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDL3WHITESOLDIERS_short')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_CDL3WHITESOLDIERS_long,self.set_CDL3WHITESOLDIERS_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDL3WHITESOLDIERS_short,self.set_CDL3WHITESOLDIERS_short,510),
                                                                ])),
                                                                width=550,
