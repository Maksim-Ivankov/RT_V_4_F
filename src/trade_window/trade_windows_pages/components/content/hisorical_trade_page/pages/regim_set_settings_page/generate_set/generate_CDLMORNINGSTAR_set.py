...
class Generate_CDLMORNINGSTAR_set(ft.UserControl):
    ...
    def input_strat_CDLMORNINGSTAR_penetration(self,e):
        self.set_CDLMORNINGSTAR_penetration = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLMORNINGSTAR_penetration':str(e.control.value)})
    def input_CDLMORNINGSTAR_long(self,e):
        self.set_CDLMORNINGSTAR_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLMORNINGSTAR_long':str(e.control.value)})
    def input_CDLMORNINGSTAR_short(self,e):
        self.set_CDLMORNINGSTAR_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLMORNINGSTAR_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDLMORNINGSTAR_set):
            os.remove(path_ini_CDLMORNINGSTAR_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'penetration':str(random.choice(self.set_CDLMORNINGSTAR_penetration.split(',')))},path_ini_CDLMORNINGSTAR_set)
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLMORNINGSTAR_long.split(',')))},path_ini_CDLMORNINGSTAR_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLMORNINGSTAR_short.split(',')))},path_ini_CDLMORNINGSTAR_set)
    ...
    def print_page(self):
        ...
        self.set_CDLMORNINGSTAR_penetration = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLMORNINGSTAR_penetration')
        self.set_CDLMORNINGSTAR_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLMORNINGSTAR_long')
        self.set_CDLMORNINGSTAR_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLMORNINGSTAR_short')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_strat_CDLMORNINGSTAR_penetration,self.set_CDLMORNINGSTAR_penetration,510),
                                                                        ft.Container(ft.Text('Процент сигнала в лонг',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLMORNINGSTAR_long,self.set_CDLMORNINGSTAR_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLMORNINGSTAR_short,self.set_CDLMORNINGSTAR_short,510),
                                                                ])),
                                                                width=550,
