...
class Generate_CDLHAMMER_set(ft.UserControl):
    def __init__(self,btn_close):
        super().__init__()
        self.btn_close = btn_close
        
        
    def input_CDLHAMMER_long(self,e):
        self.set_CDLHAMMER_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLHAMMER_long':str(e.control.value)})
    def input_CDLHAMMER_short(self,e):
        self.set_CDLHAMMER_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLHAMMER_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDLHAMMER_set):
            os.remove(path_ini_CDLHAMMER_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLHAMMER_long.split(',')))},path_ini_CDLHAMMER_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLHAMMER_short.split(',')))},path_ini_CDLHAMMER_set)
    ...
    def print_page(self):
        ...
        self.set_CDLHAMMER_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLHAMMER_long')
        self.set_CDLHAMMER_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLHAMMER_short')
                                        ...
                                        ft.Container(
                                            ft.Container(
                                                    ...
                                                    ft.Column(
                                                        controls=[
                                                            ft.Container(
                                                                ft.Container(ft.Text('Молоток',color=c_blue,),bgcolor=c_yelow,padding=5,margin=ft.margin.only(bottom=-10),border=ft.border.all(1,c_white))
                                                            ),
                                                            ft.Container(
                                                                ft.Container(
                                                                        ft.Column(controls=[
                                                                        ft.Container(ft.Text('Процент сигнала в лонг',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLHAMMER_long,self.set_CDLHAMMER_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLHAMMER_short,self.set_CDLHAMMER_short,510),
                                                                ])),
                                                                width=550,
