...
class Generate_CDLEVENINGDOJISTAR_set(ft.UserControl):
    ...
    def input_strat_CDLEVENINGDOJISTAR_penetration(self,e):
        self.set_CDLEVENINGDOJISTAR_penetration = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLEVENINGDOJISTAR_penetration':str(e.control.value)})
    def input_CDLEVENINGDOJISTAR_long(self,e):
        self.set_CDLEVENINGDOJISTAR_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLEVENINGDOJISTAR_long':str(e.control.value)})
    def input_CDLEVENINGDOJISTAR_short(self,e):
        self.set_CDLEVENINGDOJISTAR_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLEVENINGDOJISTAR_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDLEVENINGDOJISTAR_set):
            os.remove(path_ini_CDLEVENINGDOJISTAR_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'penetration':str(random.choice(self.set_CDLEVENINGDOJISTAR_penetration.split(',')))},path_ini_CDLEVENINGDOJISTAR_set)
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLEVENINGDOJISTAR_long.split(',')))},path_ini_CDLEVENINGDOJISTAR_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLEVENINGDOJISTAR_short.split(',')))},path_ini_CDLEVENINGDOJISTAR_set)
    ...
    def print_page(self):
        ...
        self.set_CDLEVENINGDOJISTAR_penetration = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLEVENINGDOJISTAR_penetration')
        self.set_CDLEVENINGDOJISTAR_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLEVENINGDOJISTAR_long')
        self.set_CDLEVENINGDOJISTAR_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLEVENINGDOJISTAR_short')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_strat_CDLEVENINGDOJISTAR_penetration,self.set_CDLEVENINGDOJISTAR_penetration,510),
                                                                        ft.Container(ft.Text('Процент сигнала в лонг',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLEVENINGDOJISTAR_long,self.set_CDLEVENINGDOJISTAR_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLEVENINGDOJISTAR_short,self.set_CDLEVENINGDOJISTAR_short,510),
                                                                ])),
                                                                width=550,
