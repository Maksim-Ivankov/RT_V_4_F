...
class CDLDARKCLOUDCOVER_table(ft.UserControl):
    def __init__(self):
        super().__init__()
    ...
    def print_page(self):
        ...
        config_general_set.read(path_ini_CDLDARKCLOUDCOVER_set)
        count = 1
        for i in config_general_set.sections():
            datas_print.append(
                ft.DataRow(cells=[
                    ft.DataCell(ft.Text(count,text_align='center',color=c_white,size=12,width=20)),
                    ft.DataCell(ft.Text(config_general_set.get(i, 'penetration'),text_align='center',color=c_white,size=12,width=80)),   
            ...
            ft.Container(
                ft.Column(controls=[
                    ft.DataTable(columns=[
                        ...
                        ft.DataColumn(ft.Text('Процент проникновения одной свечи внутри другой свечи',text_align='center',color=c_white,size=12)),
