...
class Generate_CDLENGULFING_set(ft.UserControl):
    def __init__(self,btn_close):
        super().__init__()
        self.btn_close = btn_close
        
    def input_CDLENGULFING_long(self,e):
        self.set_CDLENGULFING_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLENGULFING_long':str(e.control.value)})
    def input_CDLENGULFING_short(self,e):
        self.set_CDLENGULFING_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLENGULFING_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDLENGULFING_set):
            os.remove(path_ini_CDLENGULFING_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLENGULFING_long.split(',')))},path_ini_CDLENGULFING_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLENGULFING_short.split(',')))},path_ini_CDLENGULFING_set)
    ...
    def print_page(self):
        ...
        self.set_CDLENGULFING_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLENGULFING_long')
        self.set_CDLENGULFING_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLENGULFING_short')
                                        ...
                                        ft.Container(
                                            ft.Container(
                                                    ...
                                                    ft.Column(
                                                        controls=[
                                                            ft.Container(
                                                                ft.Container(ft.Text('Шаблон поглощения',color=c_blue,),bgcolor=c_yelow,padding=5,margin=ft.margin.only(bottom=-10),border=ft.border.all(1,c_white))
                                                            ),
                                                            ft.Container(
                                                                ft.Container(
                                                                        ft.Column(controls=[
                                                                        ft.Container(ft.Text('Процент сигнала в лонг',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLENGULFING_long,self.set_CDLENGULFING_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLENGULFING_short,self.set_CDLENGULFING_short,510),
                                                                ])),
                                                                width=550,
