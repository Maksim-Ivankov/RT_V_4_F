...
class Generate_KAMA_set(ft.UserControl):
    ...
    def input_strat_KAMA_timeperiod(self,e):
        self.set_KAMA_timeperiod = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_KAMA_timeperiod':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_KAMA_set):
            os.remove(path_ini_KAMA_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'timeperiod':str(random.choice(self.set_KAMA_timeperiod.split(',')))},path_ini_KAMA_set)
    ...
    def print_page(self):
        ...
        self.set_KAMA_timeperiod = config.get('param_trade_historical_trade_svobodniy_freym', 'set_KAMA_timeperiod')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_strat_KAMA_timeperiod,self.set_KAMA_timeperiod,510),
                                                                ])),
                                                                width=550,
