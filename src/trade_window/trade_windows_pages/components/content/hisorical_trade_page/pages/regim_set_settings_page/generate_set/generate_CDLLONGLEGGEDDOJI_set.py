...
class Generate_CDLLONGLEGGEDDOJI_set(ft.UserControl):
    ...
    def input_CDLLONGLEGGEDDOJI_long(self,e):
        self.set_CDLLONGLEGGEDDOJI_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLLONGLEGGEDDOJI_long':str(e.control.value)})
    def input_CDLLONGLEGGEDDOJI_short(self,e):
        self.set_CDLLONGLEGGEDDOJI_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLLONGLEGGEDDOJI_short':str(e.control.value)})
    ...
    def btn_generate(self,e):
        ...
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLLONGLEGGEDDOJI_long.split(',')))},path_ini_CDLLONGLEGGEDDOJI_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLLONGLEGGEDDOJI_short.split(',')))},path_ini_CDLLONGLEGGEDDOJI_set)
    ...
    def print_page(self):
        ...
        self.set_CDLLONGLEGGEDDOJI_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLLONGLEGGEDDOJI_long')
        self.set_CDLLONGLEGGEDDOJI_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLLONGLEGGEDDOJI_short')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_CDLLONGLEGGEDDOJI_long,self.set_CDLLONGLEGGEDDOJI_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLLONGLEGGEDDOJI_short,self.set_CDLLONGLEGGEDDOJI_short,510),
                                                                ])),
                                                                width=550,
