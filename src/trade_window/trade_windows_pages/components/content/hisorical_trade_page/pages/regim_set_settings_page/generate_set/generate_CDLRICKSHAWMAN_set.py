...
class Generate_CDLRICKSHAWMAN_set(ft.UserControl):
    ...
    def input_CDLRICKSHAWMAN_long(self,e):
        self.set_CDLRICKSHAWMAN_long = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLRICKSHAWMAN_long':str(e.control.value)})
    def input_CDLRICKSHAWMAN_short(self,e):
        self.set_CDLRICKSHAWMAN_short = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_CDLRICKSHAWMAN_short':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_CDLRICKSHAWMAN_set):
            os.remove(path_ini_CDLRICKSHAWMAN_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'long':str(random.choice(self.set_CDLRICKSHAWMAN_long.split(',')))},path_ini_CDLRICKSHAWMAN_set)
            Save_config(str(i)+'_section',{'short':str(random.choice(self.set_CDLRICKSHAWMAN_short.split(',')))},path_ini_CDLRICKSHAWMAN_set)
    ...
    def print_page(self):
        ...
        self.set_CDLRICKSHAWMAN_long = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLRICKSHAWMAN_long')
        self.set_CDLRICKSHAWMAN_short = config.get('param_trade_historical_trade_svobodniy_freym', 'set_CDLRICKSHAWMAN_short')
                                                        ...
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ...
                                                                        Input(self.input_CDLRICKSHAWMAN_long,self.set_CDLRICKSHAWMAN_long,510),
                                                                        ft.Container(ft.Text('Процент сигнала в шорт',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_CDLRICKSHAWMAN_short,self.set_CDLRICKSHAWMAN_short,510),
                                                                ])),
                                                                width=550,
