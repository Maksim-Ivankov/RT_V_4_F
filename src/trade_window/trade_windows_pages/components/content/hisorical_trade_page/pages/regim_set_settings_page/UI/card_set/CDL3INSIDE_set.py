...
class CDL3INSIDE_set(ft.UserControl):
    ...
    def print_page(self):
        ...
        config.read(path_ini_CDL3INSIDE_set)
                    ...
                    ft.Column(
                        controls=[
                            ...
                            ft.Container(
                                ft.Column(controls=[
                                    text_info,
                                    ft.Container(ft.ElevatedButton(content = ft.Text('Сгенерировать новый сет настроек',size=12,text_align='center'),data='CDL3INSIDE',on_click=self.callback,bgcolor=c_yelow,color=c_blue,style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=0))),alignment=ft.alignment.center,padding=ft.padding.only(top=10)),
                                ]),
                                width=280,
