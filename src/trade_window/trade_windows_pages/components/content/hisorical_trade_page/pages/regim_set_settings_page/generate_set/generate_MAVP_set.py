...
class Generate_MAVP_set(ft.UserControl):
    ...
    def input_MAVP_minperiod(self,e):
        self.set_MAVP_minperiod = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_MAVP_minperiod':str(e.control.value)})
    def input_kMAVP_maxperiod(self,e):
        self.set_MAVP_maxperiod = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_MAVP_maxperiod':str(e.control.value)})
    def input_MAVP_matype(self,e):
        self.set_MAVP_matype = e.control.value
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_MAVP_matype':str(e.control.value)})
        
    

    def btn_generate(self,e):
        if os.path.exists(path_ini_MAVP_set):
            os.remove(path_ini_MAVP_set)
        for i in range(1,int(self.how_mach_settings)+1):
            Save_config(str(i)+'_section',{'minperiod':str(random.choice(self.set_MAVP_minperiod.split(',')))},path_ini_MAVP_set)
            Save_config(str(i)+'_section',{'maxperiod':str(random.choice(self.set_MAVP_maxperiod.split(',')))},path_ini_MAVP_set)
            Save_config(str(i)+'_section',{'matype':str(random.choice(self.set_MAVP_matype.split(',')))},path_ini_MAVP_set)
    ...
    def print_page(self):
        ...
        self.set_MAVP_minperiod = config.get('param_trade_historical_trade_svobodniy_freym', 'set_MAVP_minperiod')
        self.set_MAVP_maxperiod = config.get('param_trade_historical_trade_svobodniy_freym', 'set_MAVP_maxperiod')
        self.set_MAVP_matype = config.get('param_trade_historical_trade_svobodniy_freym', 'set_MAVP_matype')
                                                    ...
                                                    ft.Column(
                                                        controls=[
                                                            ...
                                                            ft.Container(
                                                                ft.Container(
                                                                        ft.Column(controls=[
                                                                        ft.Container(ft.Text('Минимальный период',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_MAVP_minperiod,self.set_MAVP_minperiod,510),
                                                                        ft.Container(ft.Text('Максимальный период',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_kMAVP_maxperiod,self.set_MAVP_maxperiod,510),
                                                                        ft.Container(ft.Text('Тип скользящей средней',size=12,color=c_white,text_align='center'),width=510),
                                                                        Input(self.input_MAVP_matype,self.set_MAVP_matype,510),
                                                                ])),
                                                                width=550,
