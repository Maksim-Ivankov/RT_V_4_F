...
class Component_volume_min(ft.UserControl):
    ...
    # выбираем фикс или динамик1
    def chech_fiks_dinamo(self,e):
        Save_config('param_trade_historical_trade_svobodniy_freym',{'set_general_regim_volume_min':e.control.data})
    ...
    def print_component(self):
        ...
        self.btn_click = config.get('param_trade_historical_trade_svobodniy_freym', 'set_general_regim_volume_min')
        ...
        if self.btn_click == 'fiks':
            ...
            self.input_celka_text = '50000,70000,120000,200000,500000,1000000'
        elif self.btn_click == 'dinamo':
            self.text_info = 'Процент объёма в свечке от дневного, %'
            self.input_celka_text = '0.01,0.03,0.05,0.07,0.09'

        self.component_volume_min = ft.Column(
            controls=[
                ft.Container(ft.Container(ft.Text('Объём торгов мин ',color=c_blue,),bgcolor=c_yelow,padding=5,margin=ft.margin.only(bottom=-10),border=ft.border.all(1,c_white)),),
                ft.Container(
                    ft.Column(
                        controls=[
        ...
        return self.component_volume_min
